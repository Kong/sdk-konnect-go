// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: 3bb782d00f51

package components

import (
	"github.com/Kong/sdk-konnect-go/internal/utils"
	"github.com/Kong/sdk-konnect-go/optionalnullable"
)

type JWTAlgorithm string

const (
	JWTAlgorithmEs256   JWTAlgorithm = "ES256"
	JWTAlgorithmEs256K  JWTAlgorithm = "ES256K"
	JWTAlgorithmEs384   JWTAlgorithm = "ES384"
	JWTAlgorithmEs512   JWTAlgorithm = "ES512"
	JWTAlgorithmEsb256  JWTAlgorithm = "ESB256"
	JWTAlgorithmEsb320  JWTAlgorithm = "ESB320"
	JWTAlgorithmEsb384  JWTAlgorithm = "ESB384"
	JWTAlgorithmEsb512  JWTAlgorithm = "ESB512"
	JWTAlgorithmEsp256  JWTAlgorithm = "ESP256"
	JWTAlgorithmEsp384  JWTAlgorithm = "ESP384"
	JWTAlgorithmEsp512  JWTAlgorithm = "ESP512"
	JWTAlgorithmEd25519 JWTAlgorithm = "Ed25519"
	JWTAlgorithmEd448   JWTAlgorithm = "Ed448"
	JWTAlgorithmEdDsa   JWTAlgorithm = "EdDSA"
	JWTAlgorithmHs256   JWTAlgorithm = "HS256"
	JWTAlgorithmHs384   JWTAlgorithm = "HS384"
	JWTAlgorithmHs512   JWTAlgorithm = "HS512"
	JWTAlgorithmPs256   JWTAlgorithm = "PS256"
	JWTAlgorithmPs384   JWTAlgorithm = "PS384"
	JWTAlgorithmPs512   JWTAlgorithm = "PS512"
	JWTAlgorithmRs256   JWTAlgorithm = "RS256"
	JWTAlgorithmRs384   JWTAlgorithm = "RS384"
	JWTAlgorithmRs512   JWTAlgorithm = "RS512"
)

func (e JWTAlgorithm) ToPointer() *JWTAlgorithm {
	return &e
}

// IsExact returns true if the value matches a known enum value, false otherwise.
func (e *JWTAlgorithm) IsExact() bool {
	if e != nil {
		switch *e {
		case "ES256", "ES256K", "ES384", "ES512", "ESB256", "ESB320", "ESB384", "ESB512", "ESP256", "ESP384", "ESP512", "Ed25519", "Ed448", "EdDSA", "HS256", "HS384", "HS512", "PS256", "PS384", "PS512", "RS256", "RS384", "RS512":
			return true
		}
	}
	return false
}

type JWTConsumer struct {
	ID *string `json:"id,omitempty"`
}

func (j *JWTConsumer) GetID() *string {
	if j == nil {
		return nil
	}
	return j.ID
}

type Jwt struct {
	Algorithm optionalnullable.OptionalNullable[JWTAlgorithm] `default:"HS256" json:"algorithm"`
	Consumer  optionalnullable.OptionalNullable[JWTConsumer]  `json:"consumer,omitempty"`
	// Unix epoch when the resource was created.
	CreatedAt optionalnullable.OptionalNullable[int64] `json:"created_at,omitempty"`
	// A string representing a UUID (universally unique identifier).
	ID           optionalnullable.OptionalNullable[string] `json:"id,omitempty"`
	Key          optionalnullable.OptionalNullable[string] `json:"key,omitempty"`
	RsaPublicKey optionalnullable.OptionalNullable[string] `json:"rsa_public_key,omitempty"`
	Secret       optionalnullable.OptionalNullable[string] `json:"secret,omitempty"`
	// A set of strings representing tags.
	Tags optionalnullable.OptionalNullable[[]string] `json:"tags,omitempty"`
}

func (j Jwt) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(j, "", false)
}

func (j *Jwt) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &j, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (j *Jwt) GetAlgorithm() optionalnullable.OptionalNullable[JWTAlgorithm] {
	if j == nil {
		return nil
	}
	return j.Algorithm
}

func (j *Jwt) GetConsumer() optionalnullable.OptionalNullable[JWTConsumer] {
	if j == nil {
		return nil
	}
	return j.Consumer
}

func (j *Jwt) GetCreatedAt() optionalnullable.OptionalNullable[int64] {
	if j == nil {
		return nil
	}
	return j.CreatedAt
}

func (j *Jwt) GetID() optionalnullable.OptionalNullable[string] {
	if j == nil {
		return nil
	}
	return j.ID
}

func (j *Jwt) GetKey() optionalnullable.OptionalNullable[string] {
	if j == nil {
		return nil
	}
	return j.Key
}

func (j *Jwt) GetRsaPublicKey() optionalnullable.OptionalNullable[string] {
	if j == nil {
		return nil
	}
	return j.RsaPublicKey
}

func (j *Jwt) GetSecret() optionalnullable.OptionalNullable[string] {
	if j == nil {
		return nil
	}
	return j.Secret
}

func (j *Jwt) GetTags() optionalnullable.OptionalNullable[[]string] {
	if j == nil {
		return nil
	}
	return j.Tags
}
