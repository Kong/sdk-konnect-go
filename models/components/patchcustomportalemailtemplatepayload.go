// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: 34614769245b

package components

import (
	"github.com/Kong/sdk-konnect-go/internal/utils"
	"github.com/Kong/sdk-konnect-go/optionalnullable"
)

// PatchCustomPortalEmailTemplatePayload - Update a custom portal email template.
type PatchCustomPortalEmailTemplatePayload struct {
	Content optionalnullable.OptionalNullable[EmailTemplateContent] `json:"content,omitempty"`
	// Whether the email template is enabled or disabled for a portal
	Enabled *bool `default:"true" json:"enabled"`
}

func (p PatchCustomPortalEmailTemplatePayload) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(p, "", false)
}

func (p *PatchCustomPortalEmailTemplatePayload) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &p, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (p *PatchCustomPortalEmailTemplatePayload) GetContent() optionalnullable.OptionalNullable[EmailTemplateContent] {
	if p == nil {
		return nil
	}
	return p.Content
}

func (p *PatchCustomPortalEmailTemplatePayload) GetEnabled() *bool {
	if p == nil {
		return nil
	}
	return p.Enabled
}
