// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: a178973d7a94

package components

import (
	"github.com/Kong/sdk-konnect-go/optionalnullable"
)

// Pem - A keypair in PEM format.
type Pem struct {
	PrivateKey *string `json:"private_key,omitempty"`
	PublicKey  *string `json:"public_key,omitempty"`
}

func (p *Pem) GetPrivateKey() *string {
	if p == nil {
		return nil
	}
	return p.PrivateKey
}

func (p *Pem) GetPublicKey() *string {
	if p == nil {
		return nil
	}
	return p.PublicKey
}

// Set - The id (an UUID) of the key-set with which to associate the key.
type Set struct {
	ID *string `json:"id,omitempty"`
}

func (s *Set) GetID() *string {
	if s == nil {
		return nil
	}
	return s.ID
}

// A Key object holds a representation of asymmetric keys in various formats. When Kong or a Kong plugin requires a specific public or private key to perform certain operations, it can use this entity.
type Key struct {
	// Unix epoch when the resource was created.
	CreatedAt optionalnullable.OptionalNullable[int64] `json:"created_at,omitempty"`
	// A string representing a UUID (universally unique identifier).
	ID optionalnullable.OptionalNullable[string] `json:"id,omitempty"`
	// A JSON Web Key represented as a string.
	Jwk optionalnullable.OptionalNullable[string] `json:"jwk,omitempty"`
	// A unique identifier for a key.
	Kid string `json:"kid"`
	// The name to associate with the given keys.
	Name optionalnullable.OptionalNullable[string] `json:"name,omitempty"`
	// A keypair in PEM format.
	Pem optionalnullable.OptionalNullable[Pem] `json:"pem,omitempty"`
	// The id (an UUID) of the key-set with which to associate the key.
	Set optionalnullable.OptionalNullable[Set] `json:"set,omitempty"`
	// An optional set of strings associated with the Key for grouping and filtering.
	Tags optionalnullable.OptionalNullable[[]string] `json:"tags,omitempty"`
	// Unix epoch when the resource was last updated.
	UpdatedAt optionalnullable.OptionalNullable[int64] `json:"updated_at,omitempty"`
	// X.509 certificate SHA-1 thumbprint.
	X5t optionalnullable.OptionalNullable[string] `json:"x5t,omitempty"`
}

func (k *Key) GetCreatedAt() optionalnullable.OptionalNullable[int64] {
	if k == nil {
		return nil
	}
	return k.CreatedAt
}

func (k *Key) GetID() optionalnullable.OptionalNullable[string] {
	if k == nil {
		return nil
	}
	return k.ID
}

func (k *Key) GetJwk() optionalnullable.OptionalNullable[string] {
	if k == nil {
		return nil
	}
	return k.Jwk
}

func (k *Key) GetKid() string {
	if k == nil {
		return ""
	}
	return k.Kid
}

func (k *Key) GetName() optionalnullable.OptionalNullable[string] {
	if k == nil {
		return nil
	}
	return k.Name
}

func (k *Key) GetPem() optionalnullable.OptionalNullable[Pem] {
	if k == nil {
		return nil
	}
	return k.Pem
}

func (k *Key) GetSet() optionalnullable.OptionalNullable[Set] {
	if k == nil {
		return nil
	}
	return k.Set
}

func (k *Key) GetTags() optionalnullable.OptionalNullable[[]string] {
	if k == nil {
		return nil
	}
	return k.Tags
}

func (k *Key) GetUpdatedAt() optionalnullable.OptionalNullable[int64] {
	if k == nil {
		return nil
	}
	return k.UpdatedAt
}

func (k *Key) GetX5t() optionalnullable.OptionalNullable[string] {
	if k == nil {
		return nil
	}
	return k.X5t
}
