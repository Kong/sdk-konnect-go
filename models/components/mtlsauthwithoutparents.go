// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: 82d94499024f

package components

import (
	"github.com/Kong/sdk-konnect-go/optionalnullable"
)

type MTLSAuthWithoutParentsCaCertificate struct {
	ID *string `json:"id,omitempty"`
}

func (m *MTLSAuthWithoutParentsCaCertificate) GetID() *string {
	if m == nil {
		return nil
	}
	return m.ID
}

type MTLSAuthWithoutParentsConsumer struct {
	ID *string `json:"id,omitempty"`
}

func (m *MTLSAuthWithoutParentsConsumer) GetID() *string {
	if m == nil {
		return nil
	}
	return m.ID
}

type MTLSAuthWithoutParents struct {
	CaCertificate *MTLSAuthWithoutParentsCaCertificate                              `json:"ca_certificate,omitempty"`
	Consumer      optionalnullable.OptionalNullable[MTLSAuthWithoutParentsConsumer] `json:"consumer,omitempty"`
	// Unix epoch when the resource was created.
	CreatedAt optionalnullable.OptionalNullable[int64] `json:"created_at,omitempty"`
	// A string representing a UUID (universally unique identifier).
	ID          optionalnullable.OptionalNullable[string] `json:"id,omitempty"`
	SubjectName string                                    `json:"subject_name"`
	// A set of strings representing tags.
	Tags optionalnullable.OptionalNullable[[]string] `json:"tags,omitempty"`
}

func (m *MTLSAuthWithoutParents) GetCaCertificate() *MTLSAuthWithoutParentsCaCertificate {
	if m == nil {
		return nil
	}
	return m.CaCertificate
}

func (m *MTLSAuthWithoutParents) GetConsumer() optionalnullable.OptionalNullable[MTLSAuthWithoutParentsConsumer] {
	if m == nil {
		return nil
	}
	return m.Consumer
}

func (m *MTLSAuthWithoutParents) GetCreatedAt() optionalnullable.OptionalNullable[int64] {
	if m == nil {
		return nil
	}
	return m.CreatedAt
}

func (m *MTLSAuthWithoutParents) GetID() optionalnullable.OptionalNullable[string] {
	if m == nil {
		return nil
	}
	return m.ID
}

func (m *MTLSAuthWithoutParents) GetSubjectName() string {
	if m == nil {
		return ""
	}
	return m.SubjectName
}

func (m *MTLSAuthWithoutParents) GetTags() optionalnullable.OptionalNullable[[]string] {
	if m == nil {
		return nil
	}
	return m.Tags
}
