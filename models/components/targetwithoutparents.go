// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: ce9c657aa222

package components

import (
	"github.com/Kong/sdk-konnect-go/internal/utils"
	"github.com/Kong/sdk-konnect-go/optionalnullable"
)

// TargetWithoutParentsUpstream - The unique identifier or the name of the upstream for which to update the target.
type TargetWithoutParentsUpstream struct {
	ID *string `json:"id,omitempty"`
}

func (t *TargetWithoutParentsUpstream) GetID() *string {
	if t == nil {
		return nil
	}
	return t.ID
}

// TargetWithoutParents - A target is an ip address/hostname with a port that identifies an instance of a backend service. Every upstream can have many targets, and the targets can be dynamically added, modified, or deleted. Changes take effect on the fly. To disable a target, post a new one with `weight=0`; alternatively, use the `DELETE` convenience method to accomplish the same. The current target object definition is the one with the latest `created_at`.
type TargetWithoutParents struct {
	// Unix epoch when the resource was created.
	CreatedAt optionalnullable.OptionalNullable[float64] `json:"created_at,omitempty"`
	// Whether to use this target only as backup or not.
	Failover optionalnullable.OptionalNullable[bool] `default:"false" json:"failover"`
	// A string representing a UUID (universally unique identifier).
	ID optionalnullable.OptionalNullable[string] `json:"id,omitempty"`
	// An optional set of strings associated with the Target for grouping and filtering.
	Tags optionalnullable.OptionalNullable[[]string] `json:"tags,omitempty"`
	// The target address (ip or hostname) and port. If the hostname resolves to an SRV record, the `port` value will be overridden by the value from the DNS record.
	Target *string `json:"target"`
	// Unix epoch when the resource was last updated.
	UpdatedAt optionalnullable.OptionalNullable[float64] `json:"updated_at,omitempty"`
	// The unique identifier or the name of the upstream for which to update the target.
	Upstream optionalnullable.OptionalNullable[TargetWithoutParentsUpstream] `json:"upstream,omitempty"`
	// The weight this target gets within the upstream loadbalancer (`0`-`65535`). If the hostname resolves to an SRV record, the `weight` value will be overridden by the value from the DNS record.
	Weight optionalnullable.OptionalNullable[int64] `default:"100" json:"weight"`
}

func (t TargetWithoutParents) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(t, "", false)
}

func (t *TargetWithoutParents) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &t, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (t *TargetWithoutParents) GetCreatedAt() optionalnullable.OptionalNullable[float64] {
	if t == nil {
		return nil
	}
	return t.CreatedAt
}

func (t *TargetWithoutParents) GetFailover() optionalnullable.OptionalNullable[bool] {
	if t == nil {
		return nil
	}
	return t.Failover
}

func (t *TargetWithoutParents) GetID() optionalnullable.OptionalNullable[string] {
	if t == nil {
		return nil
	}
	return t.ID
}

func (t *TargetWithoutParents) GetTags() optionalnullable.OptionalNullable[[]string] {
	if t == nil {
		return nil
	}
	return t.Tags
}

func (t *TargetWithoutParents) GetTarget() *string {
	if t == nil {
		return nil
	}
	return t.Target
}

func (t *TargetWithoutParents) GetUpdatedAt() optionalnullable.OptionalNullable[float64] {
	if t == nil {
		return nil
	}
	return t.UpdatedAt
}

func (t *TargetWithoutParents) GetUpstream() optionalnullable.OptionalNullable[TargetWithoutParentsUpstream] {
	if t == nil {
		return nil
	}
	return t.Upstream
}

func (t *TargetWithoutParents) GetWeight() optionalnullable.OptionalNullable[int64] {
	if t == nil {
		return nil
	}
	return t.Weight
}
