// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: 11b7f19c6866

package operations

import (
	"github.com/Kong/sdk-konnect-go/models/components"
	"net/http"
)

type GetPortalIdentityProviderRequest struct {
	// ID of the portal.
	PortalID string `pathParam:"style=simple,explode=false,name=portalId"`
	// ID of the identity provider.
	ID string `pathParam:"style=simple,explode=false,name=id"`
}

func (g *GetPortalIdentityProviderRequest) GetPortalID() string {
	if g == nil {
		return ""
	}
	return g.PortalID
}

func (g *GetPortalIdentityProviderRequest) GetID() string {
	if g == nil {
		return ""
	}
	return g.ID
}

type GetPortalIdentityProviderResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// An identity provider configuration. This response represents the configuration of a specific identity provider, which can be either OIDC or SAML.
	//
	PortalIdentityProvider *components.PortalIdentityProvider
}

func (g *GetPortalIdentityProviderResponse) GetContentType() string {
	if g == nil {
		return ""
	}
	return g.ContentType
}

func (g *GetPortalIdentityProviderResponse) GetStatusCode() int {
	if g == nil {
		return 0
	}
	return g.StatusCode
}

func (g *GetPortalIdentityProviderResponse) GetRawResponse() *http.Response {
	if g == nil {
		return nil
	}
	return g.RawResponse
}

func (g *GetPortalIdentityProviderResponse) GetPortalIdentityProvider() *components.PortalIdentityProvider {
	if g == nil {
		return nil
	}
	return g.PortalIdentityProvider
}
