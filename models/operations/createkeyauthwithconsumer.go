// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: 732fa0b02297

package operations

import (
	"github.com/Kong/sdk-konnect-go/models/components"
	"net/http"
)

type CreateKeyAuthWithConsumerRequest struct {
	// The UUID of your control plane. This variable is available in the Konnect manager.
	ControlPlaneID string `pathParam:"style=simple,explode=false,name=controlPlaneId"`
	// Consumer ID for nested entities
	ConsumerIDForNestedEntities string `pathParam:"style=simple,explode=false,name=ConsumerIdForNestedEntities"`
	// Description of new API-key for creation
	KeyAuthWithoutParents *components.KeyAuthWithoutParents `request:"mediaType=application/json"`
}

func (c *CreateKeyAuthWithConsumerRequest) GetControlPlaneID() string {
	if c == nil {
		return ""
	}
	return c.ControlPlaneID
}

func (c *CreateKeyAuthWithConsumerRequest) GetConsumerIDForNestedEntities() string {
	if c == nil {
		return ""
	}
	return c.ConsumerIDForNestedEntities
}

func (c *CreateKeyAuthWithConsumerRequest) GetKeyAuthWithoutParents() *components.KeyAuthWithoutParents {
	if c == nil {
		return nil
	}
	return c.KeyAuthWithoutParents
}

type CreateKeyAuthWithConsumerResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// Successfully created API-key
	KeyAuth *components.KeyAuth
}

func (c *CreateKeyAuthWithConsumerResponse) GetContentType() string {
	if c == nil {
		return ""
	}
	return c.ContentType
}

func (c *CreateKeyAuthWithConsumerResponse) GetStatusCode() int {
	if c == nil {
		return 0
	}
	return c.StatusCode
}

func (c *CreateKeyAuthWithConsumerResponse) GetRawResponse() *http.Response {
	if c == nil {
		return nil
	}
	return c.RawResponse
}

func (c *CreateKeyAuthWithConsumerResponse) GetKeyAuth() *components.KeyAuth {
	if c == nil {
		return nil
	}
	return c.KeyAuth
}
