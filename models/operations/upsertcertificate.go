// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: 35b7c56ad8cd

package operations

import (
	"github.com/Kong/sdk-konnect-go/models/components"
	"net/http"
)

type UpsertCertificateRequest struct {
	// ID of the Certificate to lookup
	CertificateID string `pathParam:"style=simple,explode=false,name=CertificateId"`
	// The UUID of your control plane. This variable is available in the Konnect manager.
	ControlPlaneID string `pathParam:"style=simple,explode=false,name=controlPlaneId"`
	// Description of the Certificate
	CertificateRequest components.CertificateRequest `request:"mediaType=application/json"`
}

func (u *UpsertCertificateRequest) GetCertificateID() string {
	if u == nil {
		return ""
	}
	return u.CertificateID
}

func (u *UpsertCertificateRequest) GetControlPlaneID() string {
	if u == nil {
		return ""
	}
	return u.ControlPlaneID
}

func (u *UpsertCertificateRequest) GetCertificateRequest() components.CertificateRequest {
	if u == nil {
		return components.CertificateRequest{}
	}
	return u.CertificateRequest
}

type UpsertCertificateResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// Successfully upserted Certificate
	Certificate *components.Certificate
}

func (u *UpsertCertificateResponse) GetContentType() string {
	if u == nil {
		return ""
	}
	return u.ContentType
}

func (u *UpsertCertificateResponse) GetStatusCode() int {
	if u == nil {
		return 0
	}
	return u.StatusCode
}

func (u *UpsertCertificateResponse) GetRawResponse() *http.Response {
	if u == nil {
		return nil
	}
	return u.RawResponse
}

func (u *UpsertCertificateResponse) GetCertificate() *components.Certificate {
	if u == nil {
		return nil
	}
	return u.Certificate
}
