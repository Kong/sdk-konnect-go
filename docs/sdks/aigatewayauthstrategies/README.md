...
* [ListAiGatewayAuthStrategyUsage](#listaigatewayauthstrategyusage) - List AI Gateway Auth Strategy Usage
...
## ListAiGatewayAuthStrategyUsage

Returns the entities that reference a specific AI Gateway auth strategy, so dependencies can be identified before the auth strategy is deleted.

Each entity list is limited to the first N records per entity type.


### Example Usage

<!-- UsageSnippet language="go" operationID="list-ai-gateway-auth-strategy-usage" method="get" path="/v1/ai-gateways/{gatewayId}/auth-strategies/{authStrategyIdOrName}/usage" -->
```go
package main
...
func main() {
    ...
    res, err := s.AIGatewayAuthStrategies.ListAiGatewayAuthStrategyUsage(ctx, "bf138ba2-c9b1-4229-b268-04d9d8a6410b", "my-entity-name")
    if err != nil {
        log.Fatal(err)
    }
    if res.ListAIGatewayAuthStrategyUsageResponse != nil {
        // handle response
    }
}
```
...
**[*operations.ListAiGatewayAuthStrategyUsageResponse](../../models/operations/listaigatewayauthstrategyusageresponse.md), error**
