...
```go
sdkkonnectgo.WithServerURL("https://api.example.com")
```
...
```go
sdkkonnectgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
sdkkonnectgo.WithServerIndex(1)
```
...
```go
sdkkonnectgo.WithClient(httpClient)
```
...
```go
sdkkonnectgo.WithSecuritySource(/* ... */)
```
...
```go
sdkkonnectgo.WithTimeout(30 * time.Second)
```
