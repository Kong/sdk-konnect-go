...
```go
unitCost := components.CreateUnitCostManual(components.BillingFeatureManualUnitCost{/* values here */})
```

### BillingFeatureLLMUnitCost

```go
unitCost := components.CreateUnitCostLlm(components.BillingFeatureLLMUnitCost{/* values here */})
```
