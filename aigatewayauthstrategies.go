...
// ListAiGatewayAuthStrategyUsage - List AI Gateway Auth Strategy Usage
...
func (s *AIGatewayAuthStrategies) ListAiGatewayAuthStrategyUsage(ctx context.Context, gatewayID string, authStrategyIDOrName string, opts ...operations.Option) (*operations.ListAiGatewayAuthStrategyUsageResponse, error) {
	request := operations.ListAiGatewayAuthStrategyUsageRequest{
		GatewayID:            gatewayID,
		AuthStrategyIDOrName: authStrategyIDOrName,
	}
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/v1/ai-gateways/{gatewayId}/auth-strategies/{authStrategyIdOrName}/usage", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	hookCtx := hooks.HookContext{
		...
		BaseURL:          baseURL,
		Context:          ctx,
		OperationID:      "list-ai-gateway-auth-strategy-usage",
		OAuth2Scopes:     nil,
		SecuritySource:   s.sdkConfiguration.Security,
	}
	...
	res := &operations.ListAiGatewayAuthStrategyUsageResponse{
		StatusCode:  httpRes.StatusCode,
		ContentType: httpRes.Header.Get("Content-Type"),
		RawResponse: httpRes,
	}
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.ListAIGatewayAuthStrategyUsageResponse
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.ListAIGatewayAuthStrategyUsageResponse = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
